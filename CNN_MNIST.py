'''		Convolutional Neural Network Package for MNIST hand written digit recognization.
'''
...
def initialize_network(self, input_size, output_size):
    depth = 1  # Channel output = 20		#20x20 kernel = 5	#5x5


'pixels to slide at a time 2x2'
patch_size = 2  # 2x2 'Number of sliding operation' strides = 2
...
def n_elements_excluding_first_dimention(self, tensor): shape = tensor.shape[1:]
...
net.train()  # set the net in training mode n_iterations = 1
for itr in range(n_iterations):  # loop over the dataset multiple times current_loss = 0.0
    for i, data in enumerate(training_data, 0): inputs, labels = data
    optimizer.zero_grad()
outputs = net(inputs)
...
if (i % 100) == 99:
# start_index = i + 1 #end_index = i * batch_size
current_loss = (current_loss / 100)
# print(log._if+ 'TRAINING > START_INDEX : %d, END_INDEX : %d,
LOSS: %.3
f' % (start_index, end_index, current_loss)) print(log._if+ '
CURRENT
LOSS: %.3
f' % (current_loss)) current_loss = 0.0
...
# Set Network to Evaluation Mode net.eval()
correct = 0
total = 0
with torch.no_grad():  # Set all gradient flag to false for data in test_data:
    images, labels = data
    outputs = net(images)
    '''
...
def test_cnn(net, test_data):  # iterate over the test set dataiter = iter(test_data)
    images, labels = dataiter.next()


print(log._if + str(images.type()))
num_disp = 5
# perform a prediction for a few examples net.eval() # set the net in prediction mode
with torch.no_grad():  # don't keep track of gradients print(log._if+ str(images.shape))
...
def ClassifyImage(net, image): net.eval()


with torch.no_grad():  # don't keep track of gradients # output contains certainties for each class #print(log._if+ str(image.shape))
    outputs = net(image[:, :, :, :])
_, predicted = torch.max(outputs.data, 1)
fig, axes = plt.subplots(1, 1, figsize=(12,
                                        8))  # , sharex=True, sharey=True) axes.imshow((image[0, 0, :, :]).numpy(), cmap='gray', vmin=0, vmax=1) axes.axis('off')
