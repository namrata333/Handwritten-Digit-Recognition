...
t = transforms
'''	Convert Image from range [0, 1] to range [-1 to 1] image = (image - n_mean)/n_std	'''
n_mean = 0.5
n_std = 0.5
...
def Load_MNIST_Data(data_object, batch_size): print(log._st + "LOADING MNIST DATA")
...
def save_image(numpy_array, file_name): image_name = file_name + str(".png")
...
def StoreDataAsImage(mnist_data, dfolder):
    try:
        os.mkdir(dfolder) except OSError as exc:
...
'''Full Download : 938'''  # no_records_to_store = len(mnist_data) '''Only 64 Images Download''' no_records_to_store = 1
# Iterate Over MNIST DATA
for i, data in enumerate(mnist_data, 0): if
    (i >= no_records_to_store):
break
images, labels = data
for j in range(len(images)): file_name =

dfolder + str("/") + file_base + "_" + str(labels[j].item()) + "_" + str(i) + "_" + str(j)
...
'''
mnist_path = './data' image_path = './images' training_batch_size = 64
test_batch_size = 1000
training_object = Download_MNIST_TrainingData(mnist_path) test_object	= Download_MNIST_TestData(mnist_path)
training_data		= Load_MNIST_Data( training_object, training_batch_size ) test_data	= Load_MNIST_Data( test_object,	test_batch_size	) StoreDataAsImage(training_data, image_path)'''
